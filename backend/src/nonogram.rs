...
pub fn search_nonograms(req: &FetchRequest) -> Result<Vec<u32>, Box<dyn std::error::Error>> {
    ...
    // Map the user-facing size label to the site's category slug
    // xsmall≈5×5, small≈10×15, medium≈15×20, large≈20×30, xlarge≈30×45
    ...
    // Pick a pseudo-random page between 1 and 8
    ...
    eprintln!("[nonogram] starting HTTP fetch…");
    let body = fetch_html(&url)?;
    eprintln!("[nonogram] HTTP fetch complete, {} bytes", body.len());
...
fn parse_ids(html: &str, difficulty: u32) -> Result<Vec<u32>, Box<dyn std::error::Error>> {
    ...
    ids.dedup();
    eprintln!("[nonogram] IDs found: {}", ids.len());

    if ids.is_empty() {
        return Err("No puzzles found on page".into());
    }
...
fn parse_puzzle(html: &str, id: u32, is_bw: bool) -> Result<NonogramPuzzle, Box<dyn std::error::Error>> {
    ...
    // Title from <h1>, falling back to the page <title>
    ...
    let grid = extract_grid(&scripts)
        .ok_or("Could not find var d=[[...]] in page")?;
    ...
    eprintln!("[nonogram] grid {}×{}, palette {} color(s)", width, height, palette.len());
...
fn line_clues(line: &[u32], is_bw: bool) -> Vec<ClueEntry> {
    ...
    if run > 0 { clues.push(ClueEntry { count: run, color_idx: col }); }
    // An empty line is represented as a single zero-clue
